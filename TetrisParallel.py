...
def update(screen):
    ...
    if all(tetris.game_over for tetris in TETRIS_GAMES) or (time_limit != -1 and time_elapsed % time_limit == 0):
        ...
        for parent in parents:
            TetrisScoreList.append(parent[1].score)
            HeightList.append(parent[0].weight_height)
            HolesList.append(parent[0].weight_holes)
            BumpinessList.append(parent[0].weight_bumpiness)
            LineClearList.append(parent[0].weight_line_clear)

        for k, v in TetrisScore.items():
            v.append(TetrisScoreList[k-1])

        GenList.append(gen_generation)

        plt.plot(GenList, TetrisScore[1], label='Agente 1')
        plt.plot(GenList, TetrisScore[2], label='Agente 2')
        plt.plot(GenList, TetrisScore[3], label='Agente 3')
        plt.plot(GenList, TetrisScore[4], label='Agente 4')
        plt.xlabel('Corridas')
        plt.ylabel('Puntaje')
        plt.legend()
        plt.show()
        ...
        ScoreDataFrame.update({"height_weight": HeightList, "hole_weight": HolesList, "bumpiness_weight": BumpinessList, "lineclear_weight": LineClearList})
        pandas.DataFrame(ScoreDataFrame).to_csv("run{}.csv".format(gen_generation))
...
if __name__ == "__main__":
    ...
    for _ in range(GAME_COUNT):
        ...
        AGENTS.append(GeneticAgentComplete())
